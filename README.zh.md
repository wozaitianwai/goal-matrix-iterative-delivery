...
`loop-governance.json` 是 approval 和 publish policy 的机器门禁真源。`STATE.md` 只做人读视图，不重复 approval env、受保护路径或 publish pattern。人读状态复制机器 policy 值时，audit 会报告 `stateGovernanceDuplication`。
