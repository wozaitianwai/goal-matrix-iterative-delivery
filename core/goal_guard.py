...
EVENT_CONTEXTS = {
    ...
    "Stop": "Before completion: one loop step must have verification, checkpoint/status evidence, and push history policy if publishing. Next loop: select next pending goal, mark blocked, or state no remaining goal.",
}
...
- Execute: apply Ponytail ladder after tracing the real flow.
...
def main():
    ...
    if args.cmd == "gate":
        return gate_decision(args.phase, gate_input(args.root, args.file))
    ...
    # ponytail: text audit is heuristic; replace it with assistant-output hooks
